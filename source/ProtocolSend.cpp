...
{
	g_ErrorReport.Write("[CProtocolSend::ConnectServer] Starting connection to %s:%d\r\n", IP, Port);
	this->SocketConnect = new CustomClient;
	g_ErrorReport.Write("[CProtocolSend::ConnectServer] CustomClient created\r\n");

	if (this->SocketConnect->Connect(IP,Port))
	{
		g_ErrorReport.Write("[CProtocolSend::ConnectServer] Connect() returned true\r\n");
		...
		// Добавляем проверку реального подключения
		// Ждем немного, чтобы соединение установилось
		g_ErrorReport.Write("[CProtocolSend::ConnectServer] Sleeping 100ms...\r\n");
		Sleep(100);
		g_ErrorReport.Write("[CProtocolSend::ConnectServer] Checking IsConnected()...\r\n");
		
		if (this->SocketConnect->IsConnected())
		{
			g_ErrorReport.Write("[CProtocolSend::ConnectServer] IsConnected() returned true - connection successful\r\n");
			return true;
		}
		else
		{
			// Соединение не установилось
			g_ErrorReport.Write("[CProtocolSend::ConnectServer] IsConnected() returned false - connection failed\r\n");
			delete this->SocketConnect;
			this->SocketConnect = nullptr;
			return false;
		}
	}
	g_ErrorReport.Write("[CProtocolSend::ConnectServer] Connect() returned false\r\n");
	return false;
}
...
void CProtocolSend::SendRequestLogInNew(char* account, char* password)
{
	...
	for (int i = 0; i < SIZE_PROTOCOLVERSION; i++)
		pMsg.ClientVersion[i] = Version[i];
...
void CProtocolSend::RecvJoinServerNew(PMSG_CONNECT_CLIENT_RECV* pMsg )
{
	...
	std::cout << "Login" << HeroKey << pMsg->ClientVersion[0] << pMsg->ClientVersion[1] << pMsg->ClientVersion[2] << pMsg->ClientVersion[3] << pMsg->ClientVersion[4] << pMsg->ClientVersion[5] << pMsg->ClientVersion[6] << "\n";

	g_ConsoleDebug->Write(MCD_NORMAL, "[Connect to Server] %d = %02x %02x %02x %02x %02x %02x %02x",HeroKey,pMsg->index[0],pMsg->index[1], pMsg->ClientVersion[0], pMsg->ClientVersion[1],pMsg->ClientVersion[2],pMsg->ClientVersion[3],pMsg->ClientVersion[4],pMsg->ClientVersion[5],pMsg->ClientVersion[6]);
    ...
    else
    {
        ...
        for(int i=0;i<SIZE_PROTOCOLVERSION;i++)
        {
            if(Version[i] != pMsg->ClientVersion[i])
            {
