...
BYTE Version[SIZE_PROTOCOLVERSION] = {'1', '.', '0', '4', '.', '0', '5'};
...
{
	g_ErrorReport.Write( "> CreateSocket() called with %s:%d\r\n", IpAddr, Port);
	BOOL bResult = TRUE;
	...
	if( SocketClient.Connect(IpAddr,Port,WM_ASYNCSELECTMSG) != 1 )
	{
...
{
	g_ErrorReport.Write( "> ReceiveServerList() called\r\n");
	...
	g_ErrorReport.Write( "> About to show ServerSelWin\r\n");
	CUIMng& rUIMng = CUIMng::Instance();
	if (!rUIMng.m_CreditWin.IsShow())
	{
		g_ErrorReport.Write( "> Showing ServerSelWin\r\n");
		rUIMng.ShowWin(&rUIMng.m_ServerSelWin);
		rUIMng.m_ServerSelWin.UpdateDisplay();
		g_ErrorReport.Write( "> Showing LoginMainWin\r\n");
		rUIMng.ShowWin(&rUIMng.m_LoginMainWin);
		g_ErrorReport.Write( "> Windows shown successfully\r\n");
	}
...
void ReceiveServerConnect(BYTE* ReceiveBuffer) //Recebe informa��o do ConnectServer sobre a sala e envia a conex�o para a sala escolhida
{
	...
	g_ErrorReport.Write("[ReceiveServerConnect] About to check NEW_PROTOCOL_SYSTEM...\r\n");
	// SocketClient.Close(); // УБРАНО! Не закрываем сокет ConnectServer

#ifndef NEW_PROTOCOL_SYSTEM
	g_ErrorReport.Write("[ReceiveServerConnect] Using OLD protocol system\r\n");
	// Убираем принудительную установку флага - флаг будет установлен только после FD_CONNECT
	if (CreateSocket(IP, Data->Port))
	{
		g_ErrorReport.Write("CreateSocket() returned TRUE - waiting for FD_CONNECT event\r\n");
		// g_bGameServerConnected = TRUE; // ← УБРАНО! Флаг установится только после FD_CONNECT
	}
	else
	{
		g_ErrorReport.Write("CreateSocket() returned FALSE - connection failed\r\n");
	}
#else
	g_ErrorReport.Write("[ReceiveServerConnect] Using NEW protocol system\r\n");
	// gProtocolSend.DisconnectServer(); // ← УБРАНО! Не отключаемся от ConnectServer

	g_ErrorReport.Write("[ReceiveServerConnect] About to call gProtocolSend.ConnectServer...\r\n");
	if (gProtocolSend.ConnectServer(IP,Data->Port))
	{
		g_ErrorReport.Write("[ReceiveServerConnect] gProtocolSend.ConnectServer returned TRUE\r\n");
		g_bGameServerConnected = TRUE;
		g_ConsoleDebug->Write(MCD_NORMAL, " > ProtocolSend Connect");
	}
	else
	{
		g_ErrorReport.Write("[ReceiveServerConnect] gProtocolSend.ConnectServer returned FALSE\r\n");
	}
#endif
	
	g_ErrorReport.Write("[ReceiveServerConnect] About to create chat message...\r\n");
	char Text[100];
	g_ErrorReport.Write("[ReceiveServerConnect] GlobalText[481] = %s\r\n", GlobalText[481]);
	g_ErrorReport.Write("[ReceiveServerConnect] IP = %s, Port = %d\r\n", IP, Data->Port);
	g_ErrorReport.Write("[ReceiveServerConnect] About to call sprintf...\r\n");
	sprintf(Text,GlobalText[481],IP,Data->Port);
	g_ErrorReport.Write("[ReceiveServerConnect] sprintf completed\r\n");
	g_ErrorReport.Write("[ReceiveServerConnect] Chat message created: %s\r\n", Text);
	g_ErrorReport.Write("[ReceiveServerConnect] About to add chat message to list...\r\n");
	if (g_pChatListBox != nullptr)
	{
		g_pChatListBox->AddText("", Text, SEASON3B::TYPE_SYSTEM_MESSAGE);
		g_ErrorReport.Write("[ReceiveServerConnect] Chat message added to list successfully\r\n");
	}
	else
	{
		g_ErrorReport.Write("[ReceiveServerConnect] ERROR: g_pChatListBox is NULL!\r\n");
	}
		g_ErrorReport.Write("[ReceiveServerConnect] Function completed successfully\r\n");
	g_ErrorReport.Write("[ReceiveServerConnect] Exiting function...\r\n");
	g_ErrorReport.Write("[ReceiveServerConnect] CRITICAL: About to return from function - client may crash here!\r\n");
}
...
{
	g_ErrorReport.Write("[ReceiveJoinServer] Function called\r\n");
    ...
    else
    {
        ...
        // Добавляем логирование версий для отладки
        g_ErrorReport.Write("[ReceiveJoinServer] Client version: ");
        for(int i=0;i<SIZE_PROTOCOLVERSION;i++)
        {
            g_ErrorReport.Write("%02X ", Version[i]);
        }
        g_ErrorReport.Write("\\r\\n");
        
        g_ErrorReport.Write("[ReceiveJoinServer] Server version: ");
        for(int i=0;i<SIZE_PROTOCOLVERSION;i++)
        {
            g_ErrorReport.Write("%02X ", Data2->Version[i]);
        }
        g_ErrorReport.Write("\\r\\n");
        
        for(int i=0;i<SIZE_PROTOCOLVERSION;i++)
        {
            if(Version[i] != Data2->Version[i])
            {
                g_ErrorReport.Write("[ReceiveJoinServer] Version mismatch at position %d: client=%02X, server=%02X\\r\\n", i, Version[i], Data2->Version[i]);
...
void ProtocolCompiler( CWsctlc *pSocketClient, int iTranslation, int iParam)
{
	...
	while(1)
	{
		...
		{
			g_ErrorReport.Write("[ProtocolCompiler] No more packets in queue\r\n");
			break;
		}
		else 
		{
			g_ErrorReport.Write("[ProtocolCompiler] Processing packet from queue\r\n");
			BOOL bEncrypted = FALSE;
			BYTE byDec[MAX_SPE_BUFFERSIZE_];
			
			// Добавляем логирование получаемых пакетов
			g_ErrorReport.Write("[ProtocolCompiler] Received packet - Header: 0x%02X\r\n", ReceiveBuffer[0]);
			...
			if( ReceiveBuffer[0] == 0xC1 )
			{
				...
				Size     = lpHeader->Size;
				g_ErrorReport.Write("[ProtocolCompiler] C1 packet - HeadCode: 0x%02X, Size: %d\r\n", HeadCode, Size);
			}
			else if( ReceiveBuffer[0] == 0xC2 )
			{
				...
				g_ErrorReport.Write("[ProtocolCompiler] C2 packet - HeadCode: 0x%02X, Size: %d\r\n", HeadCode, Size);
			}
			else if( ReceiveBuffer[0] == 0xC3 || ReceiveBuffer[0] == 0xC4)
			{
				...
				g_ErrorReport.Write("[ProtocolCompiler] C3/C4 packet - Size: %d, DecryptedSize: %d\r\n", Size, iSize);
				
				if ( iSize < 0)
				{
					g_ErrorReport.Write("[ProtocolCompiler] Decrypt failed - sending hack check\r\n");
			...
			{
				g_ErrorReport.Write("[ProtocolCompiler] Unknown packet header: 0x%02X\r\n", ReceiveBuffer[0]);
				g_ErrorReport.HexWrite(ReceiveBuffer, 16); // Логируем первые 16 байт
				continue;
			}
...
{
	g_ErrorReport.Write("[TranslateProtocol] Processing HeadCode: 0x%02X, Size: %d, Encrypted: %d\r\n", HeadCode, Size, bEncrypted);
	switch( HeadCode )
	{
	case 0xF1:     			
		{
			g_ErrorReport.Write("[TranslateProtocol] Received GameServer packet - HeadCode: 0xF1\r\n");
			LPPHEADER_DEFAULT_SUBCODE Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
			g_ErrorReport.Write("[TranslateProtocol] GameServer SubCode: 0x%02X\r\n", Data->SubCode);
			switch( Data->SubCode )
			{
			case 0x00: //receive join server
				g_ErrorReport.Write("[TranslateProtocol] Calling ReceiveJoinServer...\r\n");
                ReceiveJoinServer(ReceiveBuffer);
				g_ErrorReport.Write("[TranslateProtocol] ReceiveJoinServer completed\r\n");
				break;
