...
{
	g_ErrorReport.Write( "> ServerSelWin::Show(%s)\r\n", bShow ? "true" : "false");
	CWin::Show(bShow);
}
...
{
	g_ErrorReport.Write( "> ShowServerBtns: m_iSelectServerBtnIndex=%d, m_icntServer=%d\r\n", 
		m_iSelectServerBtnIndex, m_icntServer);
	
	if (m_iSelectServerBtnIndex == -1)
	{
		g_ErrorReport.Write( "> No server group selected\r\n");
...
void CServerSelWin::UpdateWhileActive(double dDeltaTick)
{
	...
	frameCount++;
	if (frameCount % 60 == 0) // Логируем каждую секунду (60 FPS)
	{
		g_ErrorReport.Write( "> UpdateWhileActive: m_pSelectServerGroup=%p, m_icntServer=%d\r\n", 
			m_pSelectServerGroup, m_icntServer);
	}
	int i;
	...
	for( i=0 ; i<SSW_SERVER_G_MAX ; i++ )
	{
		...
		{
			g_ErrorReport.Write( "> Server group button %d clicked\r\n", i);
			if(m_iSelectServerBtnIndex != -1 )
			{
			...
			g_ErrorReport.Write( "> Selected server group %d\r\n", i);
	...
	for( i=0 ; i<m_icntServer ; i++ )
	{
		...
		{
			g_ErrorReport.Write( "> Server button %d clicked\r\n", i);
			pServerInfo = m_pSelectServerGroup->GetServerInfo(i);

			if( pServerInfo == NULL )
			{
				g_ErrorReport.Write( "> ServerInfo is NULL\r\n");
				return;
			}

			g_ErrorReport.Write( "> Server percent: %d\r\n", pServerInfo->m_iPercent);
