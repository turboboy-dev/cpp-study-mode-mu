...
void OpenTextData()
{
	...
	// Загружаем .mspro файлы (кастомные функции Encoder)
	if (g_MsproLoader.LoadMsproFiles())
	{
		char logMsg[256];
		sprintf(logMsg, "Loaded %d .mspro files successfully", g_MsproLoader.GetLoadedFilesCount());
		g_ErrorReport.Write("> %s\r\n", logMsg);
	}
	else
	{
		g_ErrorReport.Write("> Failed to load .mspro files: %s\r\n", g_MsproLoader.GetLastError());
	}
}
