...
{\
	g_ErrorReport.Write( "> SendRequestServerAddress() called with index %d\r\n", p_Index);\
	CStreamPacketEngine spe;\
	spe.Init( 0xC1, 0xF4);\
...
{	
	g_ErrorReport.Write("[SendCheck] Called - g_bGameServerConnected = %d\r\n", g_bGameServerConnected);
	if ( !g_bGameServerConnected)
	{
		g_ErrorReport.Write("[SendCheck] Skipping - no GameServer connection\r\n");
		return;
	}

	g_ErrorReport.Write("[SendCheck] Sending authentication packet\r\n");
...
__forceinline bool SendRequestEquipmentItem(int iSrcType,int iSrcIndex, ITEM* pItem, int iDstType,int iDstIndex)
{
	...
	spe << BYTECAST(BYTE, iSrcType) << BYTECAST(BYTE, iSrcIndex) << BYTECAST(BYTE, pItem->Type) << BYTECAST(BYTE, pItem->Level)
		<< BYTECAST(BYTE,pItem->Durability) << BYTECAST(BYTE,pItem->Option1) << BYTECAST(BYTE,pItem->ExtOption)
		<< splitType << spareBits
		<< socketBits[0] << socketBits[1] << socketBits[2] << socketBits[3] << socketBits[4]
