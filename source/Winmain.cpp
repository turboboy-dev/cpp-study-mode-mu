...
// Прототипы функций
void LoadPluginsFromEncoderIni();
...
void CheckHack( void)
{
	// Не отправляем SendCheck до аутентификации
	if (CurrentProtocolState < RECEIVE_LOG_IN_SUCCESS)
	{
		g_ErrorReport.Write("[CheckHack] Skipping SendCheck - not authenticated yet (CurrentProtocolState: %d)\r\n", CurrentProtocolState);
		return;
	}
...
LONG FAR PASCAL WndProc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_ASYNCSELECTMSG :
		g_ErrorReport.Write("[WndProc] WM_ASYNCSELECTMSG received - Event: %d, Error: %d\r\n", WSAGETSELECTEVENT(lParam), WSAGETSELECTERROR(lParam));
		switch( WSAGETSELECTEVENT( lParam ) )
		{
		case FD_CONNECT :
			g_ErrorReport.Write("[WndProc] FD_CONNECT event received - setting g_bGameServerConnected = TRUE\r\n");
			g_bGameServerConnected = TRUE;
			g_ConsoleDebug->Write(MCD_NORMAL, "FD_CONNECT: GameServer connection established");
			g_ErrorReport.Write("FD_CONNECT: GameServer connection established\r\n");
			break;
		case FD_READ :
			g_ErrorReport.Write("[WndProc] FD_READ event received\r\n");
			g_ErrorReport.Write("[WndProc] About to call SocketClient.nRecv()...\r\n");
			SocketClient.nRecv();
			g_ErrorReport.Write("[WndProc] SocketClient.nRecv() completed\r\n");
			break;
		case FD_WRITE :
			g_ErrorReport.Write("[WndProc] FD_WRITE event received\r\n");
...
BOOL OpenInitFile()
{
	...
	// Загружаем плагины из Encoder.ini
	LoadPluginsFromEncoderIni();
...
// Функция загрузки плагинов из Encoder.ini
void LoadPluginsFromEncoderIni()
{
	char szIniFilePath[256] = "";
	char szCurrentDir[256];
	...
	// Формируем путь к Encoder.ini
	strcpy(szIniFilePath, szCurrentDir);
	if(szCurrentDir[strlen(szCurrentDir)-1] == '\\') 
		strcat(szIniFilePath, "Encoder.ini");
	else 
		strcat(szIniFilePath, "\\Encoder.ini");

	g_ErrorReport.Write("> Loading plugins from Encoder.ini: %s\r\n", szIniFilePath);

	// Проверяем существование файла
	FILE* fp = fopen(szIniFilePath, "r");
	if(!fp)
	{
		g_ErrorReport.Write("> ERROR: Encoder.ini not found!\r\n");
		return;
	}
	fclose(fp);

	// Загружаем плагины из секции [MainSettings]
	for(int i = 1; i <= 15; i++)
	{
		sprintf(szPluginName, "Plugin%d", i);
		GetPrivateProfileString("MainSettings", szPluginName, "", szPluginPath, sizeof(szPluginPath), szIniFilePath);
		
		if(strlen(szPluginPath) > 0)
		{
			// Формируем полный путь к плагину
			char szFullPath[256];
			strcpy(szFullPath, szCurrentDir);
			if(szCurrentDir[strlen(szCurrentDir)-1] == '\\') 
				strcat(szFullPath, "Plugins\\");
			else 
				strcat(szFullPath, "\\Plugins\\");
			strcat(szFullPath, szPluginPath);

			// Проверяем существование файла плагина
			FILE* pluginFile = fopen(szFullPath, "r");
			if(!pluginFile)
			{
				g_ErrorReport.Write("> ERROR: Plugin file not found: %s\r\n", szFullPath);
				continue;
			}
			fclose(pluginFile);

			// Загружаем плагин
			hPlugin = LoadLibrary(szFullPath);
			if(hPlugin)
			{
				g_ErrorReport.Write("> SUCCESS: Loaded plugin: %s\r\n", szPluginPath);
				
				// Пытаемся инициализировать плагин
				typedef BOOL (*InitPluginFunc)();
				InitPluginFunc initFunc = (InitPluginFunc)GetProcAddress(hPlugin, "InitializePlugin");
				if(initFunc)
				{
					if(initFunc())
						g_ErrorReport.Write("> SUCCESS: Initialized plugin: %s\r\n", szPluginPath);
					else
						g_ErrorReport.Write("> WARNING: Failed to initialize plugin: %s\r\n", szPluginPath);
				}
				else
				{
					// Пробуем другие имена функций инициализации
					initFunc = (InitPluginFunc)GetProcAddress(hPlugin, "InitPlugin");
					if(initFunc)
					{
						if(initFunc())
							g_ErrorReport.Write("> SUCCESS: Initialized plugin (InitPlugin): %s\r\n", szPluginPath);
						else
							g_ErrorReport.Write("> WARNING: Failed to initialize plugin (InitPlugin): %s\r\n", szPluginPath);
					}
									else
				{
					g_ErrorReport.Write("> INFO: No initialization function found for: %s\r\n", szPluginPath);
					
					// Плагины могут работать через DLL injection, попробуем принудительно инициализировать
					if(strstr(szPluginPath, "CEffect.dll") != NULL)
					{
						// Читаем конфиг CEffect.dll
						char szConfigPath[256];
						strcpy(szConfigPath, szCurrentDir);
						if(szCurrentDir[strlen(szCurrentDir)-1] == '\\') 
							strcat(szConfigPath, "Plugins\\CESettings.ini");
						else 
							strcat(szConfigPath, "\\Plugins\\CESettings.ini");
						
						FILE* configFile = fopen(szConfigPath, "r");
						if(configFile)
						{
							g_ErrorReport.Write("> SUCCESS: Found CEffect config: %s\r\n", szConfigPath);
							fclose(configFile);
							
							// Принудительно активируем CEffect.dll
							g_ErrorReport.Write("> ATTEMPTING: Force activation of CEffect.dll effects...\r\n");
							
							// Попытка вызвать функцию активации эффектов
							typedef void (*ActivateEffectsFunc)();
							ActivateEffectsFunc activateEffects = (ActivateEffectsFunc)GetProcAddress(hPlugin, "ActivateEffects");
							if(activateEffects)
							{
								activateEffects();
								g_ErrorReport.Write("> SUCCESS: CEffect.dll effects activated!\r\n");
							}
							else
							{
								g_ErrorReport.Write("> INFO: CEffect.dll uses automatic activation (DLL injection)\r\n");
								
								// Попробуем применить настройки вручную
								g_ErrorReport.Write("> ATTEMPTING: Manual application of CEffect settings...\r\n");
								
								// Попытка вызвать функцию применения настроек
								typedef void (*ApplySettingsFunc)(const char*);
								ApplySettingsFunc applySettings = (ApplySettingsFunc)GetProcAddress(hPlugin, "ApplySettings");
								if(applySettings)
								{
									applySettings(szConfigPath);
									g_ErrorReport.Write("> SUCCESS: CEffect settings applied manually!\r\n");
								}
								else
								{
									g_ErrorReport.Write("> INFO: CEffect settings will be applied automatically\r\n");
								}
							}
						}
						else
						{
							g_ErrorReport.Write("> WARNING: CEffect config not found: %s\r\n", szConfigPath);
						}
					}
					else if(strstr(szPluginPath, "opengl32.dll") != NULL)
					{
						// Читаем конфиг opengl32.dll
						char szConfigPath[256];
						strcpy(szConfigPath, szCurrentDir);
						if(szCurrentDir[strlen(szCurrentDir)-1] == '\\') 
							strcat(szConfigPath, "Plugins\\QeffectsGL.ini");
						else 
							strcat(szConfigPath, "\\Plugins\\QeffectsGL.ini");
						...
						if(configFile)
						{
							g_ErrorReport.Write("> SUCCESS: Found OpenGL config: %s\r\n", szConfigPath);
							fclose(configFile);
							
							// Принудительно активируем opengl32.dll
							g_ErrorReport.Write("> ATTEMPTING: Force activation of opengl32.dll effects...\r\n");
							
							// Попытка вызвать функцию активации OpenGL эффектов
							typedef void (*ActivateOGLEffectsFunc)();
							ActivateOGLEffectsFunc activateOGLEffects = (ActivateOGLEffectsFunc)GetProcAddress(hPlugin, "ActivateOGLEffects");
							if(activateOGLEffects)
							{
								activateOGLEffects();
								g_ErrorReport.Write("> SUCCESS: opengl32.dll effects activated!\r\n");
							}
							else
							{
								g_ErrorReport.Write("> INFO: opengl32.dll uses automatic activation (DLL injection)\r\n");
								
								// Попробуем применить настройки вручную
								g_ErrorReport.Write("> ATTEMPTING: Manual application of OpenGL settings...\r\n");
								
								// Попытка вызвать функцию применения настроек
								typedef void (*ApplyOGLSettingsFunc)(const char*);
								ApplyOGLSettingsFunc applyOGLSettings = (ApplyOGLSettingsFunc)GetProcAddress(hPlugin, "ApplyOGLSettings");
								if(applyOGLSettings)
								{
									applyOGLSettings(szConfigPath);
									g_ErrorReport.Write("> SUCCESS: OpenGL settings applied manually!\r\n");
								}
								else
								{
									g_ErrorReport.Write("> INFO: OpenGL settings will be applied automatically\r\n");
								}
							}
						}
						else
						{
							g_ErrorReport.Write("> WARNING: OpenGL config not found: %s\r\n", szConfigPath);
						}
					}
				}
				}
			...
			else
			{
				...
				g_ErrorReport.Write("> ERROR: Failed to load plugin %s (Error: %d)\r\n", szPluginPath, error);
			}
		}
	}
}
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int nCmdShow)
{
	...
	// Безопасное освобождение ресурсов перед закрытием
	g_ErrorReport.Write("=== CLIENT SHUTDOWN START ===\r\n");
	
	// Сначала отключаем сетевые соединения
	#ifdef NEW_PROTOCOL_SYSTEM
		gProtocolSend.DisconnectServer();
	#endif
	SocketClient.Close();
	...
	g_ErrorReport.Write("Destroying window...\r\n");
	DestroyWindow();
	
	g_ErrorReport.Write("=== CLIENT SHUTDOWN COMPLETE ===\r\n");
